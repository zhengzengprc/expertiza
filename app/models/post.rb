class Post < ActiveRecord::Base
  ...
  ## This class provides the brunt of the functionality for the Expertiza message board.
  
  # get best_post value
  def get_best(post_num)
    Post.find(:first, :conditions => ["id = ?",post_num]).best_post
  end
  
  # set best_post value
  def self.set_best(post_num)
    post = Post.find(:first, :conditions => ["id = ?",post_num])
    if post.best_post == 0    # if not checked
      post.best_post = 1      # check it...
    else                      # was already checked
      post.best_post = 0      # uncheck it...
    end
    post.save
  end
  ...
  # find the number of cheers/uncheers per user _overall_ (e.g. across all topics): 
  ...
  def self.cheers_per_user
    ...
    # 3) select entries a) by 'posts.name', summing their respective 'cheercounts' and 'uncheercounts' and 'best_post', and saving those sums in new variables 
  ...
  # find the number of cheers/uncheers per user in _a_ particular topic: 
  # returns an array of post objects
  def self.cheers_per_user_by_topic(topic)
    puts "....#{topic}"
    Post.all(:joins => :cheers,
             :group => "posts.name",
  ...
  # find the number of 'best' rating a user has gotten overall (e.g. across all topics)
  # 1. group posts table by user's names, so we can sum their total number of 'best posts' they have written
  # 2. sume the number of 
  def self.bests_per_user
    Post.all(:group => "posts.name",
             :select => "posts.name, SUM(posts.best_post) AS best_sum",
             :order => "best_sum DESC")
  end
  
  # find the number of 'best' rating a user has gotten in _a_ particular topic
  def self.bests_per_user_by_topic(topic)
    Post.all(:group => "posts.name",
             :conditions => ['topic_id = ?',topic],
             :select => "posts.name, SUM(posts.best_post) AS best_sum",
             :order => "best_sum DESC")
  ...
  # returns a list of all posts of a particular topic, nicely threaded
  def self.get_all_threads(topic)
    # get all the posts (sans replies)
    posts = Post.find_all_posts(topic)
  ...
  # a search method for time-sorted posts (e.g. posts with no parents) of a particular topic
  def self.find_all_posts(topic)
    Post.find(:all, :conditions => ["parentpost = ? AND topic_id = ?",0,topic], :order => "created_at DESC")
  end
  
  # a search method for time-sorted replies to a particular post
  # (NOTE: we don't check for topic type here, as the topic type is irrelavent:
  # we would never display a reply to a post in one topic in another topic)
  def self.find_all_replies(post)
    Post.find(:all, :conditions => ["parentpost = ?", post.id], :order => "created_at ASC")
  end
  
  
  # returns a list of posts of a topic, all containing some particular string, nicely threaded
  def self.find_some_threads(search,topic)
    # get all the posts that either contain the search term, or their replies contain the search term
    posts = Post.find_all_in_threads(search,topic) 
    thread_order = []
    posts.each do |post|
      # save the id of 'post'
      thread_order << post.id
  ...
  def self.find_all_in_threads(search,topic)
    search_condition = "%" + search + "%"
    # find all posts with the search term in the name or posttext, and that are of the right topic
    posts = find(:all, 
                         :conditions => ['((posttext LIKE ? OR name LIKE ?) AND parentpost = ?) AND topic_id = ?', search_condition, search_condition, 0, topic],
                         :order => "created_at DESC")
    posts.each {|p|"...#{p.posttext}"}
    # find all the replies with the search term in the name or posttext, here we do not care about the reply's topic
    replies = find(:all,
  ...
  def self.get_friends_threads(user,topic)    
    ...
    # get all the posts by friends, or whose replies are by friends, only in the current topic
    posts = Post.find_friends_in_threads(friends,topic)   # go find all the posts by friends. if a reply to post was by a friend, the parent, plus the rest of the thread is returned 
    thread_order = []
    posts.each do |post| 
      # save the id of 'post'
      thread_order << post.id
  ...
  # a search method for time-sorted _threads_ containing friends, of a particular topic
  # e.g. if the friend is in the reply to a thread, the parent that spawned that thread is returned 
  def self.find_friends_in_threads(friends,topic)
    
    posts = []
    replies = []
    friends.each do |f|
      # find all posts belonging to our friends of the topic in question
      posts += find(:all,           # NOTE: array1 + array2 = concatenation of array1 and array2
                  :conditions => ['name = ? AND parentpost = ? AND topic_id = ?', f.name, 0,topic],
                  :order => "created_at DESC")
      # find all the replies for this friend for the topic in question
      replies += find(:all,
                    :conditions => ['name = ? AND parentpost >= ? AND topic_id = ?', f.name, 0, topic],
                    :order => "created_at ASC")
    end
    ...
    # may now be listed in order
    #puts "parent_ids: #{posts_and_replies.each}"
    Post.find(posts_and_replies, :order => "created_at DESC")
  end
